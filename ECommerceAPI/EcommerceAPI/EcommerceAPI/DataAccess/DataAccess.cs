    ...
    public class DataAccess : IDataAccess
    {
        ...
        public Product GetProduct(int id)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT * FROM Products WHERE ProductId=" + id.ToString() + ";";
                command.CommandText = query;
        ...
        public string IsUserPresent(string email, string password)
        {
            ...
            using(SqlConnection connection = new(dbconnection))
            {
                ...
                {
                    connection.Close();
                    return "";  //Returning if user is not present in the DB
                }
                ...
                //Creating Jwt Token Secret Key
                string key = "MNU66iBl3T5rh6H52i69";
                string duration = "5"; //Duration is in minutes
                var symmetrickey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(key));
                var credentials = new SigningCredentials(symmetrickey, SecurityAlgorithms.HmacSha256);

                //Data that to be stored in token
                var claims = new[]
                {
